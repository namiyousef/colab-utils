...
try:
    ...
    print(
        (
            f"Google Drive import failed with exception {e}... "
            "continuing anyways..."
        )
    )
...
def install_package_dev_mode(repo_name, requirements="requirements.txt"):
    ...
    subprocess.check_call(
        [sys.executable, "-m", "pip", "install", "-e", base_path]
    )
    subprocess.check_call(
        [
            sys.executable,
            ...
            "install",
            "-r",
            os.path.join(base_path, requirements),
        ]
    )
...
def install_private_library(path_to_config, repo_name, branch="develop"):
    ...
    git_link = (
        f"git+https://{access_token}@github.com/"
        f"{username}/{repo_name}.git@{branch}"
    )
    try:
        output = subprocess.check_output(
            [sys.executable, "-m", "pip", "install", git_link]
        )  # security risk, output not encrypted
        del output
...
def move_to_device(data, device="cpu"):
    ...
    if torch.is_tensor(data):
        ...
        raise NotImplementedError(
            "Currently no support for tensors stored in lists."
        )
    else:
